...
Validation checkpoint: 2026-06-26 remaining Level 3 classification. This
checkpoint reran the portable fixture conformance runner and the C# conformance
runner, then made one short classification pass over Ada, Delphi/Object Pascal,
Objective-C, SQLite, Assembly, COBOL, and Scratch. No SHA3 result is substituted
for SHA-256, and no Assembly or COBOL SHA-256 implementation is claimed.

| Language | Level | Verified locally | Validation command | Current blocker | Generate / marker-complete fixture status |
|---|---:|---|---|---|---|
| Ada | 2 | No | `gnat --version`; intended `gnatmake languages/ada/tests/position_tape_tests.adb` | `gnat` is not on PATH; no locally testable exact SHA-256 path was added in the short attempt | Not locally verified |
| Assembly | 1 | No | WSL/Linux: `nasm -v`; then `sh languages/assembly/tests/verify_position_tape_100.sh` | NASM is available on Windows, but the current program targets Linux x86-64 syscalls and still requires a Linux/WSL linker/runtime path for validation; no simple tested SHA-256 path exists | Not locally verified |
...
| COBOL | 1 | No | `cobc -V` | `cobc` is not on PATH; no simple tested SHA-256 path exists | Not locally verified |
...
| Delphi/Object Pascal | 2 | No | `fpc -iV` | `fpc` is not on PATH; no locally testable exact SHA-256 path was added in the short attempt | Not locally verified |
...
| SQLite | 2 | Yes | From repo root: `Get-Content languages/sqlite/tests/position_tape_tests.sql | sqlite3`; SHA probe: `sqlite3 -batch ":memory:" "SELECT lower(hex(sha3('abc',256))); SELECT sha256('abc');"` | SQLite 3.53.2 has SHA3 but no `sha256()` function; Level 3 is blocked until exact SHA-256 is available and tested | API generation, marker-complete boundaries, validation, truncation, and direct locate verified; official fixture files not directly checked |
